...
def process_handover_file(csv_path):
    ...
    Returns:
        List of tuples (mean_before, mean_after) for each handover event
    """
    ...
    for event_id, group in df.groupby('handover_event_id'):
        ...
        if len(before_samples) > 0 and len(after_samples) > 0:
            ...
            handover_points.append((mean_before, mean_after))
...
def create_heatmap(all_points, output_path):
    ...
    scatter = ax.scatter(x, y, c='blue', s=50, alpha=0.6, 
...
def main():
    if len(sys.argv) != 3:
        print("Usage: python3 ./heatmap.py <input handover data folder> heatmap.png")
        sys.exit(1)
